...
class LinearProbeReg(torch.nn.Module):
    ...
    def __init__(self, common_dim, feature_size_list, latent_dim, gene_num):
        ...
        self.fc1 = torch.nn.Linear(common_dim * 2, latent_dim)
        self.relu = torch.nn.ReLU()
        self.fc2 = torch.nn.Linear(latent_dim, gene_num)
    ...
    def forward(self, x):
        ...
        x1, x2 = torch.split(x, self.feature_size_list, dim=-1)
        x1 = self.linear1(x1)
        x2 = self.linear2(x2)
...
def evaluate(model, criterion_list, val_loader, device, target_genes, args):
    # Evaluate the model
    model.eval()
    with torch.no_grad():
        total_loss = 0
        ...
        for _, batch in enumerate(val_loader):
            ...
            output, x1, x2 = model(WSI_features)
            regression_loss, mse_loss, coef_loss = criterion_list[0](output, RNA_expression)
            contrastive_loss = args.contrastive_weight * criterion_list[1](x1, x2)
            loss = regression_loss + contrastive_loss
...
def train_eval(model,
    ...
    criterion1 = CustomLoss(args.coef_weight)
    criterion2 = CLIPLoss(temperature=0.5)
    criterion_list = [criterion1, criterion2]
    ...
    for epoch in range(args.epochs_num):
        for batch_idx, batch in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch+1}/{args.epochs_num}"):
            ...
            regression_loss, _, _ = criterion_list[0](output, RNA_expression)
            contrastive_loss = args.contrastive_weight * criterion_list[1](x1, x2)
        ...
        train_results = evaluate(model, criterion_list, train_loader, device, target_genes, args)
...
def main():
    ...
    for version_index in range(10):
        ...
        for fold_name in fold_list:
            ...
            model = LinearProbeReg(args.common_dim, args.feature_size_list, args.latent_dim, len(target_genes))
...
argparser.add_argument('--feature_size_list',   type=int, nargs='+', help='number of extracted features from FM')
...
argparser.add_argument('--common_dim',          type=int, default=1024, help='dimension of embedded common space')
