...
class LinearProbeReg(torch.nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, feature_size_list, latent_dim, gene_num):
        ...
        self.cross_attention2 = MultiheadAttention(embed_dim=embed_dim, num_heads=num_heads, kdim=feature_size_list[1], vdim=feature_size_list[1], qdim=feature_size_list[0])
        self.fc1 = torch.nn.Linear(embed_dim*2, latent_dim)
    ...
    def forward(self, x):
        ...
        x1, x2 = torch.split(x, self.feature_size_list, dim=-1) # 'TITAN' 'PRISM'
        x12, _ = self.cross_attention1(x2, x1, x1)
        x21, _ = self.cross_attention2(x1, x2, x2)
        x12 = x12.squeeze()
        x21 = x21.squeeze()
        x_kqv = torch.cat((x12, x21), dim=1)
        x_kqv = x_kqv.squeeze()
        
        x = self.relu(self.fc1(x_kqv))
        output = self.fc2(x)
...
def main():
    ...
    for version_index in range(10):
        ...
        for fold_name in fold_list:
            ...
            model = LinearProbeReg(args.embed_dim, args.num_heads, args.feature_size_list, args.latent_dim, len(target_genes))
