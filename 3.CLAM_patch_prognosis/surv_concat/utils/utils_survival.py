...
def collate_MIL(batch):
    ...
    y_time = torch.LongTensor([int(item[1]) for item in batch])
    y_event = torch.LongTensor([item[2] for item in batch])
...
def collate_features(batch):
    # img = torch.cat([torch.tensor(item[0]) for item in batch], dim = 0)
    img = torch.cat([item[0].clone().detach() for item in batch], dim = 0)
