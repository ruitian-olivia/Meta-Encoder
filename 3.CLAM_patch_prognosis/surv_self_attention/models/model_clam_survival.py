...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, qdim=None):
        ...
        self._qkv_same_embed_dim = self.kdim == self.qdim and self.vdim == self.qdim
        ...
        if self._qkv_same_embed_dim is False:
            ...
            self.in_proj_weight = Parameter(torch.empty(3 * embed_dim, self.kdim))
...
class CLAM_SB(nn.Module):
    def __init__(self, gate = True, encoding_size = 1024, size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False):
        super(CLAM_SB, self).__init__()
        self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384]}
        size = self.size_dict[size_arg]
        
        self.cross_attention = MultiheadAttention(embed_dim=size[0], num_heads=1, kdim=encoding_size, vdim=encoding_size, qdim=encoding_size)
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
        device = h.device
        h = h.unsqueeze(0)
        h, _ = self.cross_attention(h, h, h)
        h = h.squeeze()
