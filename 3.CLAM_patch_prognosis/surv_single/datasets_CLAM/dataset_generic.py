...
class Generic_WSI_Classification_Dataset(Dataset):
    ...
    #     if len(split) > 0:
    #         mask = self.slide_data['HE_entity_submitter_id'].isin(merged_split)
    #         df_slice = self.slide_data[mask].reset_index(drop=True)
...
class Generic_MIL_Dataset(Generic_WSI_Classification_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_h5:
            if self.data_dir:
                full_path = os.path.join(data_dir, 'pt_files', '{}.pt'.format(slide_id.split('.')[0]))
                features = torch.load(full_path)
                return features, days, censor
            
            else:
                return slide_id, days, censor
