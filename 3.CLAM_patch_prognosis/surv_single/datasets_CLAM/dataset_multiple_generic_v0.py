...
class Generic_WSI_Classification_Dataset(Dataset):
    ...
    def get_split_from_df(self, all_splits, split_key='train'):
        ...
        split_list_ID = [item.split('.')[0] for item in split_list]
        if len(split) > 0:
            slide_id_prefix = self.slide_data['slide_id'].str.split('.').str[0]
            mask = slide_id_prefix.isin(split_list_ID)
            # mask = self.slide_data['slide_id'].isin(split_list_ID)
            df_slice = self.slide_data[mask].reset_index(drop=True)
            split = Generic_Split(df_slice, features_list=self.features_list, data_dir=self.data_dir, num_classes=self.num_classes)
        else:
            split = None
    ...
    def return_splits(self, from_id=True, csv_path=None):
        ...
        if from_id:
            if len(self.train_ids) > 0:
                ...
                train_split = Generic_Split(train_data, features_list=self.features_list, data_dir=self.data_dir, num_classes=self.num_classes)
            ...
            if len(self.val_ids) > 0:
                ...
                val_split = Generic_Split(val_data, features_list=self.features_list, data_dir=self.data_dir, num_classes=self.num_classes)
            ...
            if len(self.test_ids) > 0:
                ...
                test_split = Generic_Split(test_data, features_list=self.features_list, data_dir=self.data_dir, num_classes=self.num_classes)
...
class Generic_MIL_Multiple_Dataset(Generic_WSI_Classification_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features_df = merged_features_df.drop(columns=['coords1', 'coords2'])
        features = torch.from_numpy(features_df.values).float()
