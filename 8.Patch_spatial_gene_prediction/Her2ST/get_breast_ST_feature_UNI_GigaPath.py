...
for i in tqdm(range(len(SampleList))):
    ...
    sample_path = os.path.join(bench_data_root, sample_id)
    TissueList = os.listdir(sample_path)
    for tissue_id in TissueList:
        print("Tissue ID:", tissue_id)
    
        tile_h5_path = os.path.join(bench_data_root, sample_id, tissue_id, 'patches.h5')

        assert os.path.isfile(tile_h5_path)
        tile_dataset = H5HESTDataset(tile_h5_path,img_transform=transform,chunk_size=1200)
    
        expr_h5ad_path = os.path.join(bench_data_root,sample_id,tissue_id,'aligned_adata.h5ad')
        ...
        output_save_path = os.path.join(save_h5_path, sample_id)
        if not os.path.exists(output_save_path):
            os.makedirs(output_save_path)
    
        mode = 'w'
        output_path = os.path.join(output_save_path,tissue_id+'.h5')
