...
model = protein_poisson_count(embed_dim, num_heads, num_marker, feature_size_list).to(device)
...
for epoch in range(epochs):
    print("epoch:", epoch)
    loss = train(model,train_loader,optimizer,poisson_loss,device,args) 
    print("loss:", loss)
    
    train_loss[epoch] = loss
    val_loss[epoch], val_label, val_pred = valid(model,val_loader,poisson_loss,args,use_sampling=True)
