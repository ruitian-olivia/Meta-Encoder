...
class HDF5MarkerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.current_encoder is None:
            ...
            if self.current_encoder not in self.feature_ranges:
                raise ValueError(f"Unknown encoder: {self.current_encoder}. Available encoders: {self.encoder_names}")
            
            start, end = self.feature_ranges[self.current_encoder]
...
parser.add_argument(
    "--encoder_name",
    type=str,
    choices=['CHIEF', 'UNI', 'GigaPath', 'GPFM'],
...
model = protein_poisson_count(num_feature, num_marker).to(device)
