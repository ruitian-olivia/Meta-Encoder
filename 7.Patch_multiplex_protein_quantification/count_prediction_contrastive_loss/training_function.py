...
def train(model,train_loader,optimizer,CL_loss,device,args):
    model.train()

    loss_all = 0
    for _, batch in enumerate(train_loader):
        UNI_features, CHIEF_features, GigaPath_features, count = batch
        UNI_features, CHIEF_features, GigaPath_features, count = UNI_features.to(device), CHIEF_features.to(device), GigaPath_features.to(device), count.to(device)
        ...
        ordered_features = [features_type_dict[name] for name in args.encoder_list]

        optimizer.zero_grad()
        output, x1, x2, x3 = model(ordered_features[0], ordered_features[1], ordered_features[2])

        loss, poisson_loss, contrastive_loss = CL_loss(output, count, x1, x2, x3)
        print("poisson_loss:", poisson_loss.item(), "contrastive_loss:", contrastive_loss.item())
...
def valid(model,loader,CL_loss,args,use_sampling=False):
    model.eval()
...
def test(model,loader,args,use_sampling=False):
    model.eval()
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    loss = 0.  
