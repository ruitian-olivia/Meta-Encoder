...
def main():
    ...
    # NCT-CRC-HE-100K (nonorm) UNI
    root_pth = './NCT-CRC-HE-100K'

    with open(os.path.join(root_pth, args.feature_type, f'crc100knonorm_train_{args.feature_type}.pkl'), 'rb') as f:
        crc100knonorm_train = pickle.load(f)
    with open(os.path.join(root_pth, args.feature_type, f'crc100knonorm_val_{args.feature_type}.pkl'), 'rb') as f:
    ...
    UNI_train_feats = torch.Tensor(crc100knonorm_train['embeddings'])
    UNI_train_labels = torch.Tensor(crc100knonorm_train_labels_encoded).type(torch.long)

    UNI_test_feats = torch.Tensor(crc100knonorm_test['embeddings'])
    UNI_test_labels = torch.Tensor(crc100knonorm_test_labels_encoded).type(torch.long)
    
    UNI_train_dataset = CustomDataset(UNI_train_feats, UNI_train_labels)
    UNI_test_dataset = CustomDataset(UNI_test_feats, UNI_test_labels)
    
    UNI_train_loader = DataLoader(UNI_train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    UNI_test_loader = DataLoader(UNI_test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    ...
    for rep_record in range(args.rep_num):
        print("rep_record:", args.rep_num)
        output_dir = os.path.join(args.output_root,'record' ,'rep_'+str(rep_record))
        if not os.path.isdir(output_dir):
            os.makedirs(output_dir)
    
        train_sampler = torch.utils.data.sampler.RandomSampler(UNI_train_dataset, replacement=True)
        UNI_train_sampler_loader = DataLoader(UNI_train_dataset, batch_size=args.batch_size, num_workers=args.num_workers, sampler=train_sampler, pin_memory=True)
        infinite_train_loader = itertools.cycle(UNI_train_sampler_loader)

        model = LinearProbe(args.embed_dim,9)
        
        test_results = train_eval(model, infinite_train_loader, UNI_train_loader, UNI_test_loader, output_dir, args)
...
argparser.add_argument('--feature_type',        type=str, default='UNI', help='Feature type')
