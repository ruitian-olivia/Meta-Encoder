...
class LinearProbeReg(torch.nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, feature_num, gene_num):
...
def main():
    ...
    for version_index in range(10):
        ...
        for fold_name in fold_list:
            ...
            model = LinearProbeReg(args.embed_dim, args.num_heads, args.feature_num, 1)
            output_dir = os.path.join(args.output_root, args.cancer_type, f'CA_{args.version}', f'{version_index}_{fold_name}')
    ...
    test_mean_df.to_csv(os.path.join(args.output_root, args.cancer_type, f'CA_{args.version}', 'test_results_mean.csv'), float_format='%.4f')
    ...
    test_std_df.to_csv(os.path.join(args.output_root, args.cancer_type, f'CA_{args.version}', 'test_results_std.csv'), float_format='%.4f')
