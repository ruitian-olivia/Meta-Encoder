...
class SurvDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        WSI_features_item = torch.load(match_item['WSI_features_path'].tolist()[0]) # filelist记录pt文件的位置

        survival_item = match_item['Days'].tolist()[0]
        censor_item = match_item['Censor'].tolist()[0]
        
        return WSI_features_item, survival_item, censor_item
...
def evaluate(model, criterion, val_loader, val_case_id, device, args):
    ...
    with torch.no_grad():
        ...
        for _, batch in enumerate(val_loader):
            WSI_features, survival, censor = batch
            WSI_features, survival, censor = WSI_features.to(device), survival.to(device), censor.to(device)
...
def main():
    ...
    surv_df['WSI_features_path'] = surv_df.apply(lambda x: getWSIFeatures(x.HE_entity_submitter_id, WSI_features_type_path), axis = 1)   
    ...
    surv_select_df = surv_df[['TCGA_case_id', 'HE_entity_submitter_id', 'WSI_features_path', 'Days', 'Censor']]
    ...
    for version_index in range(10):
        ...
        for fold_name in fold_list:
            ...
            model = MLP_ProbeSurv(feature_num, args.latent_dim)
